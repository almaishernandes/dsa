...
app.get('/api/ranking', authGuard, async (req, res) => {
  try {
    ...
    const result = allUsers.map(u => {
      ...
      return { id: u._id, name: u.name, role: u.role, role_custom: u.roleCustom, city: u.city, parish: u.parish, movement: u.movement,
...
app.put('/api/admin/questions/:id', adminGuard, async (req, res) => {
  try {
    ...
    const n = await db.questions.updateAsync(
      { _id: req.params.id },
      { $set: { trailId: temaId, theme: theme?.trim()||'Geral',
